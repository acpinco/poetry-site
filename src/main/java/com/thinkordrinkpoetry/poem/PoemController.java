...
public class PoemController {
    ...
    public PoemController(PoemRepository poems) {
        this.poems = poems;
    }
    ...
    public List<PoemResponse> list(@AuthenticationPrincipal AuthenticatedUser user) {
        return poems.findByPoetIdOrderByUpdatedAtDesc(poetId(user)).stream()
    ...
    public PoemResponse get(@AuthenticationPrincipal AuthenticatedUser user, @PathVariable UUID poemId) {
        return response(owned(poetId(user), poemId));
    }
    ...
    public void delete(@AuthenticationPrincipal AuthenticatedUser user, @PathVariable UUID poemId) {
        poems.delete(owned(poetId(user), poemId));
    }

    private Poem owned(UUID poetId, UUID poemId) {
        return poems.findByIdAndPoetId(poemId, poetId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    }

    private UUID poetId(AuthenticatedUser user) {
        if (user == null) {
    ...
    public static PoemResponse response(Poem poem) {
